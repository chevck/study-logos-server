...
/** Known API.Bible hosts — DNS varies by network; we try each until one connects. */
...
function envBaseUrl() {
  const raw = process.env.BIBLE_API_BASE_URL?.trim();
...
function baseUrlCandidates() {
  ...
  if (resolvedBaseUrl) {
    return [
      resolvedBaseUrl,
      ...DEFAULT_BASE_URLS.filter((url) => url !== resolvedBaseUrl),
    ];
  }
  return DEFAULT_BASE_URLS;
}

function isBibleApiNetworkError(err) {
...
async function bibleApiGet(path, { headers, params } = {}) {
  ...
  for (let i = 0; i < candidates.length; i += 1) {
    ...
    try {
      const response = await axios.get(`${base}${path}`, { headers, params });
      if (!envBaseUrl()) resolvedBaseUrl = base;
      return response;
    } catch (err) {
      lastError = err;
      const hasFallback = i < candidates.length - 1 && !envBaseUrl();
      if (!hasFallback || !isBibleApiNetworkError(err)) throw err;
      if (resolvedBaseUrl === base) resolvedBaseUrl = null;
      console.warn(
        `[study-logos] Bible API host ${base} unreachable (${err.code || err.cause?.code}); trying fallback…`,
      );
    }
  }
...
async function fetchBibleList(apiKey, language = "eng") {
  const { data } = await bibleApiGet("/bibles", {
...
export async function fetchVerseText(
  ...
  const { data } = await bibleApiGet(
    `/bibles/${bibleId}/${resource}/${encodeURIComponent(resourceId)}`,
    {
      headers: { "api-key": apiKey },
      params: {
